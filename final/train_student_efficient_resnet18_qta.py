...
def train_efficient_qat(teacher_path="models/teacher_resnet50_96.00acc.pth",
                        acc_target=91.0, max_epochs=150, batch_size=128, lr=0.01,
                        momentum=0.9, weight_decay=5e-4, warmup_epochs=5,
                        T=4.0, alpha=0.7, log_path="log_efficient_qat.csv"):
    ...
    # Student
    student = EfficientResNet18()
    student.qconfig = get_default_qat_qconfig("fbgemm")
    student_prepared = prepare_qat(student.train()).to(device)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if test_acc >= acc_target:
            print(f"[*] Efficient QAT reached {acc_target}% test accuracy at epoch {epoch}")
            break
    ...
    torch.save(quantized_model.state_dict(), "models/student_efficient_resnet18_qat_best.pth")
    pd.DataFrame(log).to_csv(log_path, index=False)

    print(f"[+] QAT Training finished. Best Acc: {best_acc:.2f}%")
    print("[+] Quantized model saved to models/student_efficient_resnet18_qat_best.pth")
