import torch
from models import EfficientResNet18
from utils import prune_and_evaluate, load_dataset

# Device
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Charger le modèle entraîné EfficientResNet18
model = EfficientResNet18().to(device)
model.load_state_dict(torch.load("models/student_efficient_resnet18_best.pth"))
model.eval()

# CIFAR10 testset
_, testloader = load_dataset(batch_size=128)

# Taux de pruning à tester
ratios = [0.2, 0.4, 0.6]

# Fichier de log spécifique
logfile = "logs/prune_efficient_resnet.csv"

# Pruning et évaluation
prune_and_evaluate(model, dataloader=testloader, device=device, ratios=ratios, logfile=logfile)
